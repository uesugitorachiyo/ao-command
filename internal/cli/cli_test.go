...
func TestMonth5OperatorWorkflowReadback(t *testing.T) {
	readbackPath := filepath.Join("..", "..", "examples", "operator", "month5-workflow.ready.json")
	code, stdout, stderr := runWithFake([]string{"operator", "workflow", "--readback", readbackPath}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow exit=%d stderr=%s", code, stderr)
	}
	for _, want := range []string{
		...
		"dry_run_only=true",
		"policy_gate=human_approval_required",
		"safe_next_work=multi_repo_product_coordination",
		...
		"calls_providers=false",
		"releases_or_deploys=false",
		"exact_next_action=Start Month 5 product coordination and operator workflow hardening; do not start RSI or release work.",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("operator workflow stdout missing %q:\n%s", want, stdout)
		}
	}

	code, stdout, stderr = runWithFake([]string{"operator", "workflow", "--readback", readbackPath, "--json"}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow JSON exit=%d stderr=%s", code, stderr)
	}
	var got map[string]any
	if err := json.Unmarshal([]byte(stdout), &got); err != nil {
		t.Fatalf("invalid operator workflow JSON: %v\n%s", err, stdout)
	}
	if got["schema"] != "ao.command.operator-workflow-readback.v0.1" ||
		...
		t.Fatalf("unexpected operator workflow JSON: %#v", got)
	}
	if got["executes_work"] != false || got["approves_work"] != false || got["mutates_repositories"] != false || got["calls_providers"] != false || got["releases_or_deploys"] != false {
		t.Fatalf("operator workflow widened authority: %#v", got)
	}
}

func TestMonth6NoReleaseOperatorWorkflowReadback(t *testing.T) {
	readbackPath := filepath.Join("..", "..", "examples", "operator", "month6-release-readiness.no-release.json")
	code, stdout, stderr := runWithFake([]string{"operator", "workflow", "--readback", readbackPath}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow month6 exit=%d stderr=%s", code, stderr)
	}
	for _, want := range []string{
		"ao_command_operator_workflow=ready",
		"ao2_version=v0.5.1",
		...
		"dry_run_only=true",
		"policy_gate=human_approval_required",
		"safe_next_work=next_cycle_adoption_readiness_and_evidence_maintenance",
		"rsi_status=denied",
		"promotion_requested=false",
		"external_beta_launched=false",
		"operator_mode=read_only",
		...
		"calls_providers=false",
		"releases_or_deploys=false",
		"exact_next_action=Close Month 6 with no release; start the next planning cycle from adoption readiness and evidence maintenance.",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("operator workflow month6 stdout missing %q:\n%s", want, stdout)
		}
	}

	code, stdout, stderr = runWithFake([]string{"operator", "workflow", "--readback", readbackPath, "--json"}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow month6 JSON exit=%d stderr=%s", code, stderr)
	}
	var got map[string]any
	if err := json.Unmarshal([]byte(stdout), &got); err != nil {
		t.Fatalf("invalid operator workflow month6 JSON: %v\n%s", err, stdout)
	}
	if got["schema"] != "ao.command.operator-workflow-readback.v0.1" ||
		got["status"] != "ready" ||
		...
		got["promotion_requested"] != false ||
		got["external_beta_launched"] != false {
		t.Fatalf("unexpected operator workflow month6 JSON: %#v", got)
	}
	if got["executes_work"] != false || got["approves_work"] != false || got["mutates_repositories"] != false || got["calls_providers"] != false || got["releases_or_deploys"] != false {
		t.Fatalf("operator workflow month6 widened authority: %#v", got)
	}
}

func TestAdoptionMonth1CompatibilityGateReadback(t *testing.T) {
	readbackPath := filepath.Join("..", "..", "examples", "operator", "adoption-month1-compatibility-gate-readback.json")
	code, stdout, stderr := runWithFake([]string{"operator", "workflow", "--readback", readbackPath}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow month1 gate exit=%d stderr=%s", code, stderr)
	}
	for _, want := range []string{
		"ao_command_operator_workflow=ready",
		"ao2_version=v0.5.1",
		...
		"compatibility_gate_reason=Fresh evidence is present, but activation is not authorized.",
		"compatibility_gate_activation_authorized=false",
		"safe_next_work=operator_adoption_drills_with_refreshed_evidence",
		"rsi_status=denied",
		"promotion_requested=false",
		"external_beta_launched=false",
		"operator_mode=read_only",
		...
		if !strings.Contains(stdout, want) {
			t.Fatalf("operator workflow month1 gate stdout missing %q:\n%s", want, stdout)
		}
	}

	code, stdout, stderr = runWithFake([]string{"operator", "workflow", "--readback", readbackPath, "--json"}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow month1 gate JSON exit=%d stderr=%s", code, stderr)
	}
	var got map[string]any
	if err := json.Unmarshal([]byte(stdout), &got); err != nil {
		t.Fatalf("invalid operator workflow month1 gate JSON: %v\n%s", err, stdout)
	}
	if got["evidence_freshness"] != "fresh" ||
		...
		got["promotion_requested"] != false ||
		got["external_beta_launched"] != false {
		t.Fatalf("unexpected operator workflow month1 gate JSON: %#v", got)
	}
	if got["executes_work"] != false || got["approves_work"] != false || got["mutates_repositories"] != false || got["calls_providers"] != false || got["releases_or_deploys"] != false {
		t.Fatalf("operator workflow month1 gate widened authority: %#v", got)
	}
}
...
func TestAdoptionMonth3EvidenceMaintenanceReadback(t *testing.T) {
	readbackPath := filepath.Join("..", "..", "examples", "operator", "adoption-month3-evidence-maintenance-readback.json")
	code, stdout, stderr := runWithFake([]string{"operator", "workflow", "--readback", readbackPath}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow month3 maintenance exit=%d stderr=%s", code, stderr)
	}
	for _, want := range []string{
		"ao_command_operator_workflow=ready",
		"ao2_version=v0.5.1",
		...
		"compatibility_gate_state=ready",
		"compatibility_gate_activation_authorized=false",
		"safe_next_work=controlled_improvement_evaluation_refresh_after_maintenance",
		"support_evidence=ao2_version,platform,exact_command,expected_result,actual_result,evidence_path,approval_status,manifest_or_checksum_state,rollback_status,observation_status,sanitized_logs",
		"rsi_status=denied",
		"promotion_requested=false",
		"external_beta_launched=false",
		...
		"calls_providers=false",
		"releases_or_deploys=false",
		"exact_next_action=Close Month 3 evidence maintenance automation; recommend Month 4 controlled improvement evaluation refresh without release, beta, promotion, provider, gate activation, or RSI authority.",
	} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("operator workflow month3 maintenance stdout missing %q:\n%s", want, stdout)
		}
	}

	code, stdout, stderr = runWithFake([]string{"operator", "workflow", "--readback", readbackPath, "--json"}, &fakeRunner{})
	if code != 0 {
		t.Fatalf("operator workflow month3 maintenance JSON exit=%d stderr=%s", code, stderr)
	}
	var got map[string]any
	if err := json.Unmarshal([]byte(stdout), &got); err != nil {
		t.Fatalf("invalid operator workflow month3 maintenance JSON: %v\n%s", err, stdout)
	}
	if got["evidence_freshness"] != "fresh" ||
		got["compatibility_gate_state"] != "ready" ||
		got["compatibility_gate_activation_authorized"] != false ||
		got["safe_next_work"] != "controlled_improvement_evaluation_refresh_after_maintenance" ||
		got["rsi_status"] != "denied" ||
		got["promotion_requested"] != false ||
		got["external_beta_launched"] != false ||
		got["provider_pilot_ran"] != false {
		t.Fatalf("unexpected operator workflow month3 maintenance JSON: %#v", got)
	}
	if got["executes_work"] != false || got["approves_work"] != false || got["mutates_repositories"] != false || got["calls_providers"] != false || got["releases_or_deploys"] != false {
		t.Fatalf("operator workflow month3 maintenance widened authority: %#v", got)
	}
}
